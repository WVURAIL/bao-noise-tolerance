...
threshold, and the threshold fixes (f, r) and hence the time penalty.
    ...
    python scripts/bias_tolerance.py --bank data/fisher_bank_chime2022_pres.npz
"""
...
def main(argv=None):
    ...
    ap.add_argument("--bank", type=Path,
                    default=ROOT / "data" / "fisher_bank_chime2022_pres.npz")
    ...
    if PRES not in names:
        raise SystemExit(f"{args.bank} has no '{PRES}' row; rebuild with "
                         f"expt_overrides={{'P_res': 1.0}}")
    meta = bank.meta
    ...
    nu21 = 1420.406
    z_dtv = (nu21 / 608.0 - 1.0, nu21 / 470.0 - 1.0)
    zs = bank.zs
    ...
    for ib in dtv_bins:
        ...
        print(f"  cond(F) at 1 yr: "
              f"{condition(bank.F(ib, survey.ONSKY_YEAR_HOURS), names):.2e}\n")
