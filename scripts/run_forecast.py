...
def yrs(hours: float) -> float:
    ...
    return float(hours) / survey.ONSKY_YEAR_HOURS
...
def main() -> None:
    ...
    scen_main = {
        ...
        "uniform75_dtv": scenarios.uniform(0.75, "dtv"),
    ...
    table_scens["uniform25_dtv"] = scenarios.uniform(0.25, "dtv")
    table_scens["uniform90_dtv"] = scenarios.uniform(0.90, "dtv")
    table_scens["uniform50_all"] = scenarios.uniform(0.50, "all")
    ...
    for key, sc in table_scens.items():
        ...
        s1sky = fc.significance(sc, survey.ONSKY_YEAR_HOURS)  # 1 on-sky yr
        s2yr = fc.significance(sc, 2.0 * survey.ONSKY_YEAR_HOURS)
        rows.append(dict(
    ...
    # bin 6: z=[1.402,1.507] contains ch30 + the ch31-35 cluster
    # bin 8: z=[1.611,1.715] contains refused ch24
    bin_targets = []
    for ib in (6, 8):
    ...
    IB = 6   # z = 1.40-1.51 bin: ch30 + the ch31-35 cluster
    fracs = np.concatenate([np.arange(0.0, 0.96, 0.05), [0.97]])
    yrs_s5, yrs_b5, yrs_da = [], [], []
    for f in fracs:
        sc = scenarios.uniform(float(f), "dtv")
    ...
    plots.fig_channel_masking(meas.fractions,
                              set(scenarios.chn.REFUSED_CHANNELS),
                              outdir / "fig3_channel_masking.png")

    # ------------------------------------------ fig 4: per-bin profile
    t2 = survey.ONSKY_YEAR_HOURS
    perbin = {}
...
* **Excise it** (what the forecast prices by default): costs the overlapped
  bandwidth, a {100*5.4/24.8:.0f}% volume hit to the z=1.40-1.51 bin, a
